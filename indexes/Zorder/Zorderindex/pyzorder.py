...
class ZOrderIndexer:
    ...
    def is_in(self, z):
        r, c = self.rc(z)
        return (self.row_min <= r <= self.row_max) and (
            self.col_min <= c <= self.col_max
        )
    ...
    def next_zorder_index(self, z):
        ...
        max_v = self.max_z
        bit_position = (
            self.bit_position_init
        )  # 10000000..  bit position currently investigating
