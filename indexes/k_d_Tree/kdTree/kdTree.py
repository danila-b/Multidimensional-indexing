...
def kdtree(data, leafsize=10):
    ...
    stack = [
        (data[:, : int(ndata / 2)], idx[: int(ndata / 2)], 1, 0, True),
    ...
    while stack:
        ...
        tree[parent] = (
            (_didx, _data, _left_hrect, _right_hrect, nodeptr, right)
            if leftbranch
...
def quadratic_knn_search(data, lidx, ldata, K):
    ...
    retval = []
    sqd = ((ldata - data[:, :ndata]) ** 2).sum(
        axis=0
    )  # data.reshape((param,1)).repeat(ndata, axis=1);
...
def radius_search(tree, datapoint, radius):
    ...
    while stack:
        ...
        if leaf_idx is not None:
            ...
            distance = numpy.sqrt(
                ((leaf_data - datapoint.reshape((param, 1))) ** 2).sum(axis=0)
            )
